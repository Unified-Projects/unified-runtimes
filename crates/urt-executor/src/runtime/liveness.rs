...
/// What a teardown does when the container will not go away.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OnContainerFailure {
    /// Release the rest anyway: the entry must not outlive its runtime.
    Proceed,
    /// Stop and leave the entry in place so the next cycle tries again rather
    /// than losing track of a container that is still on the daemon.
    Retain,
}

/// The pieces a teardown touches, so a caller without an [`AppState`] (the
/// listening watchdog) can release a runtime the same way the execution path
/// does.
pub struct RuntimeTeardown<'a> {
    pub docker: &'a crate::docker::DockerManager,
    pub registry: &'a RuntimeRegistry,
    pub keep_alive_registry: &'a crate::runtime::KeepAliveRegistry,
    pub readiness: &'a dashmap::DashMap<String, std::sync::Arc<tokio::sync::Notify>>,
    pub health: &'a RuntimeHealth,
}

/// Release everything a runtime owns: its container, its keep-alive ownership,
/// any parked readiness waiters, its registry entry, its working directory and
/// its health markers.
///
/// Returns whether the teardown completed.
pub async fn release_runtime(
    targets: &RuntimeTeardown<'_>,
    name: &str,
    remove_container: bool,
    on_failure: OnContainerFailure,
) -> bool {
    if remove_container {
        match targets.docker.remove_container(name, true).await {
            Ok(()) | Err(ExecutorError::RuntimeNotFound) => {}
            Err(error) => {
                warn!(
                    runtime = %name,
                    "Failed to remove runtime container: {}",
                    error
                );
                if on_failure == OnContainerFailure::Retain {
                    return false;
                }
            }
        }
    ...
    if let Some(runtime) = targets.registry.get(name).await {
        if let Some(ref keep_alive_id) = runtime.keep_alive_id {
            targets.keep_alive_registry.unregister(keep_alive_id, name);
        }
    }

    if let Some((_, notify)) = targets.readiness.remove(name) {
        notify.notify_waiters();
    }
    targets.registry.remove(name).await;
    tokio::fs::remove_dir_all(platform::temp_dir().join(name))
        .await
        .ok();
    targets.health.clear_unreachable(name);
    targets.health.invalidate_liveness(name);
    true
}

impl AppState {
    /// The teardown pieces held on the executor state.
    pub fn teardown_targets(&self) -> RuntimeTeardown<'_> {
        RuntimeTeardown {
            docker: &self.docker,
            registry: &self.registry,
            keep_alive_registry: &self.keep_alive_registry,
...
async fn remove_dead_runtime(state: &AppState, name: &str, remove_container: bool) {
    release_runtime(
        &state.teardown_targets(),
        name,
        remove_container,
        OnContainerFailure::Proceed,
    )
    .await;
